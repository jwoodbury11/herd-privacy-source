...
test("live attestation fails closed across token, key, claim, and root attacks", async (t) => {
  const mutations = [
    ...
    ["non-empty command override", { claims: (claims) => { claims.submods.container.cmd_override = ["malicious-command"]; } }],
    ["wrong environment override shape", { claims: (claims) => { claims.submods.container.env_override = []; } }],
    ["non-empty environment override", { claims: (claims) => { claims.submods.container.env_override = { SECRET: "injected" }; } }],
